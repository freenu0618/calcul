...
class OvertimeCalculator:
    ...
    ※ 연장근로 계산 방식 (GPT 역산 로직):
      - 연장근로의 기본시급(1.0배)은 이미 기본급에 포함되어 있다고 해석
      - 명세서에는 가산분(0.5배)만 별도 표기하는 것이 분쟁 방어에 유리
    """

    # 가산율 (기본시급은 기본급에 포함, 가산분만 계산)
    OVERTIME_RATE = Decimal('0.5')    # 연장근로 0.5배 (가산분만)
    NIGHT_RATE = Decimal('0.5')       # 야간근로 0.5배 (가산분)
    HOLIDAY_RATE = Decimal('1.5')     # 휴일근로 1.5배 (휴일은 무급이므로 전체)
