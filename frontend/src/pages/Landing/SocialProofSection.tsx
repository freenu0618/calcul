 ...
 * 타겟 사용자 카드에 아바타 일러스트 추가
 */

const trustFacts = [
  { icon: 'science', number: '20개', label: '실제 급여 케이스 검증 테스트', detail: '다양한 근무형태·수당 조합을 검증' },
  { icon: 'gavel', number: '8개', label: '반영된 법률 및 규정', detail: '근로기준법, 최저임금법, 국민연금법 등' },
  { icon: 'update', number: '2026년', label: '최신 법령 및 요율 반영', detail: '최저임금 10,320원, 국민연금 4.75% 등' },
];

const targetUsers = [
  { icon: 'store', title: '소규모 사업장 대표', desc: '직원 1~10명, 노무사 없이 직접 급여를 관리하는 분',
    avatarBg: 'from-blue-400 to-blue-600', avatarIcon: 'storefront' },
  { icon: 'local_cafe', title: '카페·음식점 운영자', desc: '아르바이트생 주휴수당, 야간수당 계산이 필요한 분',
    avatarBg: 'from-amber-400 to-orange-500', avatarIcon: 'coffee' },
  { icon: 'business_center', title: '스타트업 경영지원', desc: 'HR 전담 인력 없이 급여 업무를 겸하는 분',
    avatarBg: 'from-violet-400 to-purple-600', avatarIcon: 'rocket_launch' },
];
    ...
    <section className="py-20 lg:py-28 bg-background-light">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-12">
          {targetUsers.map((u) => (
            <div key={u.title} className="flex items-start gap-4 bg-white rounded-xl p-5 border border-gray-100 hover:shadow-md hover:border-primary/20 transition-all duration-200">
              <div className={`w-12 h-12 bg-gradient-to-br ${u.avatarBg} rounded-full flex items-center justify-center flex-shrink-0 shadow-md`}>
                <span className="material-symbols-outlined text-white text-[22px]">{u.avatarIcon}</span>
              </div>
              <div>
        ...
        <div className="flex flex-wrap justify-center gap-4">
          {badges.map((b) => (
            <div key={b.label} className="inline-flex items-center gap-2 bg-white px-4 py-2 rounded-full shadow-sm border border-gray-200 text-sm text-text-sub">
