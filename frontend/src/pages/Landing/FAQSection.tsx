...
const faqs: FAQItem[] = [
  ...
  {
    question: '급여 지급일 직전에는 무엇부터 검산해야 하나요?',
    answer:
      '먼저 정산월, 입사일·퇴사일, 실제 근무일과 휴게시간, 주휴수당 대상 여부를 맞춰 보세요. 이어서 연장·야간·휴일근로, 상여·비과세 수당, 4대보험 취득·상실 시점, 부양가족 수를 같은 순서로 확인하면 실제 명세서와 계산 결과의 차이를 줄일 수 있습니다.',
  },
  {
  ...
  },
  {
    ...
    answer:
      '단기 근로자는 먼저 근로자인지 용역·사업소득 지급인지 구분해야 합니다. 근로자라면 시급, 실제 근무일·근무시간, 휴게시간, 주휴수당 대상 여부, 4대보험 적용 기간과 사업장 규모를 나눠 예상액을 확인하세요. 일용근로소득세, 고용보험, 산재보험처럼 별도 판단이 필요한 항목은 계산 결과를 참고용으로 보고 실제 지급 전 전문가 검토를 함께 진행하는 것이 안전합니다.',
  },
  {
    question: '상여나 비정기 수당이 있는 달은 어떻게 확인하나요?',
    answer:
      '상여, 성과급, 식대, 차량보조금, 연구수당처럼 매월 다르거나 비과세 한도가 있는 항목은 기본급과 분리해 보세요. 지급월에는 과세 대상 금액, 비과세 한도, 원천징수 세액, 회사별 공제 방식이 달라질 수 있으므로 특정 월 예상액은 급여 계산기에서 다시 확인하고 실제 지급 전 전문가 검토로 분리하는 것이 안전합니다.',
  },
  {
    question: '포괄임금제나 고정OT도 계산 전에 확인할 수 있나요?',
    answer:
      '가능하지만 기본급, 고정 연장수당 또는 고정수당, 포함된 연장·야간·휴일근로 시간, 실제 초과근로 시간을 먼저 분리해야 합니다. 포함 시간이 불명확하거나 실제 근로시간이 고정OT 범위를 넘는다면 PayTools 결과는 참고용 기준선으로 보고 근로계약서, 출퇴근 기록, 사업장 규모, 전문가 검토를 함께 확인하는 것이 안전합니다.',
  },
  {
    question: '첫 급여나 입사월 급여도 바로 계산할 수 있나요?',
    answer:
  ...
  {
    question: '계산 결과와 실제 급여명세서가 다르면 무엇부터 봐야 하나요?',
    answer:
      '먼저 같은 정산월과 급여유형으로 계산했는지 확인한 뒤 입사일·퇴사일, 실제 근무일, 휴게시간, 주휴수당 대상 여부, 비과세 수당, 부양가족 수, 4대보험 취득·상실 시점을 순서대로 맞춰 보세요. 회사별 공제나 소급 정산, 상여·성과급처럼 예외 항목이 있으면 PayTools 결과는 차이 원인을 좁히는 참고용으로 보고 실제 지급 판단은 회사 기준과 전문가 검토로 분리하는 것이 안전합니다.',
  },
];
